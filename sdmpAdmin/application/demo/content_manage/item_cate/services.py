...
import application.demo.content_manage.item.services
from application.demo.content_manage.item_cate import forms
...
def ItemCateList(request):
    ...
    if len(item_list) > 0:
        for item in item_list:
            # 根据站点ID查询站点信息
            itemInfo = application.demo.content_manage.item.services.ItemDetail(item.id)
            data = {
                'id': item.id,
                'name': item.name,
                'pid': item.pid,
